...
▎核心卖点
- 成分功效：{name}的核心成分、浓度、科技含量
- 质地体验：肤感、吸收速度、粘腻度、香味浓度
- 适用肤质：干皮/油皮/混合/敏感肌的适配程度
- 包装设计：是否卫生（按压/滴管/管状）、便携性

▎使用建议
1. 洁面 → 化妆水 → 精华 → 乳液/面霜（从稀到稠）
2. 精华使用时取2-3滴，掌心搓热后按压上脸（比涂抹吸收更好）
3. 防晒是最后一步（白天），SPF30+ 日常通勤足够
4. 开封后6-12个月内用完，存放避光避热

▎搭配建议
- 同品牌套装搭配：精华+面霜同系列效果联动
- 不同品牌混搭：避开成分冲突（如A醇+酸类刺激、VC+烟酰胺可搭配）
- 新手入门：洁面+精华+保湿三件套即可，简化步骤反而不容易出错

▎售后与保质
- 化妆品支持7天无理由退货（未拆封完好）
- 护肤品拆封后不支持退货（卫生原因）
- 如使用后过敏，保留外包装和购买凭证联系客服处理
""",
...
▎核心卖点
- 核心性能：{name}在减震/支撑/防护/透气等方面的技术优势
- 材质科技：核心面料的科技含量和功能特性
- 适配场景：适合什么运动类型/地形/天气条件
- 耐用性：关键部件的使用寿命和维护周期

▎选配指南
1. 确定运动类型和频率（日常健身 vs 专业训练 需求差异大）
2. 注意尺码选择：运动装备通常需要专业测量（跑鞋大半码、压缩衣看胸围）
3. 室内/室外差异：户外装备需要防风防水等级，室内则重透气
4. 季节适配：夏季速干透气，冬季防风保暖

▎使用与保养
- 跑鞋：避免连续两天穿同一双（让中底回弹恢复），退役后当走路鞋不浪费
- 冲锋衣：禁用柔顺剂（堵塞防水膜），定期喷DWR恢复防泼水
- 瑜伽垫：清水擦拭即可，禁用酒精（会腐蚀表面）
- 装备储存：干燥通风处，避免阳光直晒（加速材料老化）

▎售后保障
- 服装鞋帽7天无理由退货（吊牌完好、未穿着使用）
- 电子类运动设备（手表等）享受1年保修
- 装备类（帐篷/睡袋等）质量问题15天包换
""",
...
▎核心卖点
- 材质工艺：{name}的用材等级、工艺标准、环保认证
- 空间利用：产品在收纳/节省空间方面的设计巧思
- 智能联动：支持哪些智能家居生态（米家/HomeKit/华为鸿蒙）
- 日常体验：噪音控制、能耗水平、清洁便利性

▎选购技巧
1. 量好空间尺寸再选品（特别是柜子/沙发/床等大家具）
2. 看环保标准：E1级板材甲醛释放量合格，实木>板材但更贵
3. 色系统一：全屋搭配同一色温/色系更显高级感
4. 功能优先：不需要的花哨功能（如七彩灯光）反而增加故障点

▎养护维护
- 实木家具：每季度打一次木蜡，远离暖气片和加湿器（防开裂）
- 布艺沙发：每月吸尘，局部污渍用专用清洁剂不水洗
- 不锈钢厨具：顺着纹路擦拭，禁用钢丝球（留划痕）
- 智能设备：定期OTA固件更新，勿用湿布直接擦拭带电部件

▎售后说明
- 大件家具签收时请先验货再签收，发现破损当场拍照拒收
- 家居电器整机保修1-2年（视品类）
- 床上用品贴身类拆封后不支持退货
""",
}
...
▎护肤品选购
- 先确定肤质：油性选控油清爽型、干性选滋润保湿型、混合选水油平衡型、敏感肌选无香精酒精
- 看成分表：烟酰胺（美白）、二裂酵母（修护）、玻尿酸（补水）、水杨酸（去角质）、神经酰胺（屏障）
- 精华/面霜是核心投资，洁面/水乳可以平价
- 注意保质期：开封后6-12个月内用完，瓶身有「开封后X月」图标

▎彩妆选购
- 粉底液：选对色号最关键，试在脸颊和脖子交界处，自然光下看效果
- 口红：MAC Chili（黄皮友好）、正红色（正式场合）、豆沙色（日常通勤）
- 眼影盘：新手选4-6色大地盘，进阶可尝试暖棕/橘棕盘
- 持久度：油皮需定妆散粉+定妆喷雾双保险

▎美发工具
- 吹风机：负离子减少毛躁，2000W以上速干效果好
- 卷发棒：自动卷发器适合手残党，传统卷发棒更灵活但需练习
- 温度控制：细软发质150-170℃，粗硬发质180-210℃

▎香水选购
- EDP（淡香精）留香4-6小时，EDT（淡香水）留香2-4小时
- 花香调柔和日常、木质调成熟稳重、柑橘调清新夏日
- 试香：喷在手腕不要摩擦，过1小时后闻中后调再决定
""",
...
▎跑鞋选购
- 按跑步类型：日常慢跑选缓震型、竞速选碳板轻量、越野选抓地力强的
- 按脚型：正常足选稳定型、扁平足选支撑型、高足弓选缓震型
- 尺码：比日常鞋大半码到一码（跑步时脚会肿胀）
- 寿命：跑鞋500-800公里后缓震衰减，建议3-6个月更换

▎运动服装
- 速干面料（聚酯纤维）：吸汗快干、适合跑步/健身
- 压缩衣：促进血液循环、减少肌肉振动、运动表现+3%
- 冲锋衣：看防水指数（>5000mm日常够用、>10000mm专业级）和透湿指数
- 冬季运动：三层穿法——速干内层+抓绒中层+防风外层

▎健身器材
- 跑步机：家用选静音折叠型（<55dB），跑带宽度≥45cm，持续马力>1.5HP
- 瑜伽垫：厚度6-10mm（初学者选厚）、材质TPE（环保无味）>NBR>PVC
- 哑铃：快调式哑铃省空间，固定式哑铃更稳定，新手2-10kg区间够用

▎户外装备
- 帐篷：3季帐适合春夏秋，4季帐抗风雪；3人帐实际只能睡2人+行李
- 睡袋：棉睡袋便宜但重，羽绒睡袋轻暖但怕潮
- 登山杖：铝合金经济实惠，碳纤维轻量减震
""",
...
▎床上用品
- 面料：长绒棉>普通棉，60支以上即细腻，80支以上即奢华
- 织法：贡缎光泽顺滑、天竺棉柔软透气、磨毛冬季温暖
- 被芯：羽绒轻盈保暖、蚕丝透气贴身、大豆纤维经济实惠
- 枕芯：仰睡选中低枕(8-10cm)、侧睡选中高枕(12-14cm)、俯睡选低枕
...
▎智能家居
- 智能音箱：控制灯/空调/窗帘，支持语音场景联动
- 智能灯：色温2700K-6500K可调，支持日出唤醒/助眠模式
- 智能窗帘：轨道+电机一体安装，需预留插座
- 网关是核心：选Zigbee协议的设备需要网关，WiFi直连的设备不需要

▎清洁工具
- 吸尘器：无线手持最灵活、桶式吸力最强、扫地机器人最省心
- 拖地：蒸汽拖把杀菌效果好、平板拖日常够用、洗地机一步到位
- 玻璃清洁：磁吸双面擦窗器，高层安全又干净
""",
}
